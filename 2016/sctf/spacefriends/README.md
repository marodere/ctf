...
## Dividing graph
Pick some integer `ew > 0`. Assume that `ew` is the largest connection-weight that crosses between the two halfs. Notice that if there's an edge greater than `ew` between two nodes, both should belong to the same half. Therefore, if there's a path between two nodes consisting of edges greater than `ew`, both nodes should belong to the same half.
...
## Subset sum
How to find out if there's such set? We'll answer using dynamic programming. Let `fP[i]` be the count of nodes in the `i`'s subgraph and try to reach `w` nodes.

First of all, suppose that we're able to reach 0 nodes using no subgraphs.

Next,

- If we're able to reach `w` nodes using subgraphs `1..i-1`, then we're able to reach `w` nodes even with subgraph `i` - we're just not required to include it.
- If we're able to reach `w-fP[i]` nodes using subgraphs `1..i-1`, then we're able to reach `w` nodes using subgraph `i` by adding it to the set.
